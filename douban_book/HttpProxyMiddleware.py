...
from twisted.internet.error import TimeoutError, ConnectionRefusedError, ConnectError,ConnectInProgressError,SSLError
...
class HttpProxyMiddleware(object):
    ...
    DONT_RETRY_ERRORS = (TimeoutError, ConnectionRefusedError, ResponseNeverReceived, ConnectError,SSLError, ValueError,ConnectInProgressError,TunnelError)
    ...
    def process_response(self, request, response, spider):
        ...
        if response.status==404:
            logger.debug("The page of %s is not exists"%response.url)

        # status不是正常的200，也不是404，而且不在spider声明的正常爬取过程中可能出现的
    ...
    def process_exception(self, request, exception, spider):
        ...
        处理由于使用代理导致的连接异常
        """
        # logger.debug(u"%s exception: %s" % (self.proxyes[request.meta["proxy_index"]]["proxy"], exception))
